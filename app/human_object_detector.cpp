...
void CameraModule::generateImage(cv::dnn::Net cm_yolo_model,
 std::string cm_file_name, std::vector<std::string> cm_class_list ) {
        ...
        while (true) {
                ...
                // capture every frame in a webcam into matrix object
                cap >> cm_image_in;
                // post and preprocessing method
                cm_img = HOD.objectDetectorModel(cm_image_in,
                cm_yolo_model, cm_class_list, cm_file_name);
                // Displays image from webcam
                cv::imshow("Object Detection", cm_img);
                auto end = std::chrono::system_clock::now();
                std::chrono::duration<double> elapsed_seconds = end-start;
                std::cout << "elapsed time: "
                << elapsed_seconds.count() << std::endl;
                // Essentially the webcam's framerate
                cv::waitKey(25);
        }
}
...
cv::Mat HumanObjectDetector::
objectDetectorModel(cv::Mat image_in, cv::dnn::Net &yolo_model,
